...
Two cost modes (--cost-mode):
  ...
  api-equivalent
      ...
      Input tokens are available only for sessions with a shutdown event (23/106).
...
# Ratios derived from the 23 sessions that have full token data via shutdown events.
# Used to extrapolate input/cache tokens for the 83 sessions that only have output tokens.
...
def print_summary(by_date: dict, import_stats: dict, cost_mode: str) -> None:
    ...
    if cost_mode == "actual":
        ...
        print("  without shutdown events (78/106 sessions).")
